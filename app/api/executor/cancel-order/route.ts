...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userData.role !== 'executor') {
      return NextResponse.json({ error: 'Only executors can cancel orders' }, { status: 403 })
    }
    ...
    // Проверяем, что заказ принадлежит этому исполнителю
    if (order.executorId !== userData.id) {
    ...
    const updatedOrder = await prisma.order.update({
      ...
      data: { 
        ...
        cancellationReason: reason || 'Отменен исполнителем'
      },
      include: {
        client: {
          select: {
    ...
    await prisma.notification.create({
      data: {
        ...
        message: `Исполнитель ${userData.name} отменил ваш заказ "${order.serviceDescription}". ${reason ? `Причина: ${reason}` : ''}`,
